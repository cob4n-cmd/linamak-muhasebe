...
const emptyDebt = {
  description: '', amount: '', kdv_rate: '20', debt_date: '', due_date: '', note: ''
};
...
export default function SupplierDetail() {
  ...
  const unpaidTotal = debts.filter(d => d.status === 'unpaid').reduce((s, d) => s + Number(d.total_amount || d.amount || 0), 0);
  const paidTotal = debts.filter(d => d.status === 'paid').reduce((s, d) => s + Number(d.total_amount || d.amount || 0), 0);
              ...
              <tbody className="divide-y divide-gray-100">
                {filteredDebts.map(d => (
                  <tr key={d.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-right whitespace-nowrap font-medium">{fmt(d.total_amount)}</td>
