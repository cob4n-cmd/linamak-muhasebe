...
router.get('/:id', (req, res) => {
  const job = db.prepare(`SELECT j.*, c.name as customer_name FROM jobs j LEFT JOIN customers c ON j.customer_id = c.id WHERE j.id = ?`).get(req.params.id);
  if (!job) return res.status(404).json({ error: 'Is bulunamadi' });
  job.payments = db.prepare('SELECT * FROM payments WHERE job_id = ? ORDER BY payment_date DESC').all(req.params.id);
  job.expenses = db.prepare(`SELECT e.*, s.name as supplier_name FROM expenses e LEFT JOIN suppliers s ON e.supplier_id = s.id WHERE e.job_id = ? ORDER BY e.expense_date DESC`).all(req.params.id);
  res.json(job);
});
...
router.post('/', (req, res) => {
  ...
  if (!cid && customer_name) {
    ...
    if (existing) {
      ...
      const r = db.prepare('INSERT INTO customers (name) VALUES (?)').run(customer_name);
  ...
  const r = db.prepare(`INSERT INTO jobs (title, customer_id, description, status, invoice_status, invoice_no, contract_value, kdv_rate, contract_value_with_kdv, start_date, end_date)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(
...
router.put('/:id', (req, res) => {
  ...
  db.prepare(`UPDATE jobs SET title=?, customer_id=?, description=?, status=?, invoice_status=?, invoice_no=?, contract_value=?, kdv_rate=?, contract_value_with_kdv=?, start_date=?, end_date=?, updated_at=datetime('now','localtime') WHERE id=?`)
    .run(title, cid || null, description || '', status || 'beklemede', invoice_status || 'faturasiz', invoice_no || '', val, rate, valWithKdv, start_date || null, end_date || null, req.params.id);
