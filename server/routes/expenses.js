...
router.post('/', (req, res) => {
  ...
  const r = db.prepare(`INSERT INTO expenses (job_id, supplier_id, category, description, amount, kdv_rate, kdv_amount, total_with_kdv, expense_date, is_paid, payment_method)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(
...
router.put('/:id', (req, res) => {
  ...
  const kdv_amount = Math.round(amount * rate / 100 * 100) / 100;
  const total_with_kdv = Math.round((amount + kdv_amount) * 100) / 100;
  db.prepare(`UPDATE expenses SET job_id=?, supplier_id=?, category=?, description=?, amount=?, kdv_rate=?, kdv_amount=?, total_with_kdv=?, expense_date=?, is_paid=?, payment_method=? WHERE id=?`)
    .run(job_id || null, supplier_id || null, category, description || '', amount, rate, kdv_amount, total_with_kdv, expense_date, is_paid ? 1 : 0, payment_method || 'nakit', req.params.id);
